...
func SetupMockCliConnection(mockCtrl *gomock.Controller) *MockCliConnection {
	...
	mockConnection.EXPECT().CliCommandWithoutTerminalOutput("curl", fmt.Sprintf("/v3/apps?names=%s", AppCName)).Return([]string{
		appC,
	}, nil).AnyTimes()

	mockConnection.EXPECT().CliCommandWithoutTerminalOutput("curl", fmt.Sprintf("/v3/apps?names=%s", NotAnApp)).Return(strings.Split(errorV3, "\n"), nil).AnyTimes()
